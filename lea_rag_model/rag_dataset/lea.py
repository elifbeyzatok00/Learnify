...
def load_documents(path):
    ...
    for doc in os.listdir(path):
        ...
        # DOCX dosyalarını işleme
            ...
            # "Fizyosoft" kelimelerini "Becure" ile değiştirin
            doc_text = doc_text.replace("Fizyosoft", "Becure")
...
# Kullanım
documents = load_documents("new_dataset")
...
embedding_model_name = E5_LARGE_V2 
...
model_name = LLAMA_3_2_3B_INSTURCT 
...
## selectbox + Prompt Engineering
# İçeriğe göre istemi dinamik olarak selectbox kullanarak oluşturma fonksiyonu
def generate_prompt_selectbox(style, prompt):
    if style == "I prefer not to specify a topic":
        return prompt
    elif style == "Becure":
        return f"Give an in-depth overview of Becure's treatment approaches and technological innovations: {prompt}"
    elif style == "Diseases and Treatments":
        return f"Offer comprehensive information on relevant diseases and available treatment options, focusing on: {prompt}"
    else:
        # İşlenmemiş stiller için varsayılan davranış (özelleştirilebilir)
        return f"Provide detailed information on: {prompt}"
    
## if-else + Prompt Engineering
# İçeriğe göre istemi dinamik olarak oluşturma işlevi
def generate_prompt(prompt):
    if "Becure" in prompt or "becure" in prompt:
        return f"Give an in-depth overview of Becure's treatment approaches and technological innovations: {prompt}"
    elif "Diseases and Treatments" in prompt or "disease" in prompt or "treatment" in prompt or "rehabilitation" in prompt:
        return f"Offer comprehensive information on relevant diseases and available treatment and rehabilitation options, focusing on: {prompt}"
    else:
        # Varsayılan istem davranışı
        return prompt
    
## template_dict + Prompt 
# İçeriğe göre istemi dinamik olarak selectbox kullanarak oluşturma fonksiyonu
# Anahtar kelimelere göre farklı şablonlar tanımlayın
template_dict = {
   "Becure": "Give an in-depth overview of Becure's treatment approaches and technological innovations: {prompt}",
   "becure": "Give an in-depth overview of Becure's treatment approaches and technological innovations: {prompt}",
   "Diseases and Treatments": "Offer comprehensive information on relevant diseases and available treatment options, focusing on: {prompt}",
   "disease": "Offer comprehensive information on relevant diseases and available treatment options, focusing on: {prompt}",
   "treatment": "Offer comprehensive information on relevant diseases and available treatment options, focusing on: {prompt}",
   "rehabilitation": "Offer comprehensive information on relevant diseases and available rehabilitation options, focusing on: {prompt}"
}

# Doğru şablonu seçme işlevi
def get_template(prompt):
   for key, template in template_dict.items():
       if key in prompt:
           return template  # Burada template döndürülmeli
   return "{prompt}"  # Varsayılan davranış