...
from model import *  # Tüm fonksiyonları miras al

# Streamlit başlığı
st.title("Becure LLM Q&A App")

# Kullanıcıdan cevap stili seçimini alma
#style = st.selectbox("Select what you want to obtain information about:",
#                     ["I prefer not to specify a topic.","Becure", "Diseases and Treatments"])

# Kullanıcıdan prompt al
prompt = st.text_input("Please write your question here:")
...
if st.button("Send"):
    if prompt:
        with st.spinner("Thinking..."):
            ...
            #modified_prompt = generate_prompt_selectbox(style, prompt)
            modified_prompt = generate_prompt(prompt)
            #modified_prompt = get_template(prompt).format(prompt=prompt)
